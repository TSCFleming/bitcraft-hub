...
pub trait admin_restore_all_buildings_health {
    ...
    fn on_admin_restore_all_buildings_health(&self, callback: impl FnMut(&super::ReducerEventContext, ) + Send + 'static) -> AdminRestoreAllBuildingsHealthCallbackId;
    /// Cancel a callback previously registered by [`Self::on_admin_restore_all_buildings_health`],
    /// causing it not to run in the future.
    fn remove_on_admin_restore_all_buildings_health(&self, callback: AdminRestoreAllBuildingsHealthCallbackId);
}

impl admin_restore_all_buildings_health for super::RemoteReducers {
    fn admin_restore_all_buildings_health(&self, ) -> __sdk::Result<()> {
        self.imp.call_reducer("admin_restore_all_buildings_health", AdminRestoreAllBuildingsHealthArgs {  })
    }
    ...
    fn remove_on_admin_restore_all_buildings_health(&self, callback: AdminRestoreAllBuildingsHealthCallbackId) {
        self.imp.remove_on_reducer("admin_restore_all_buildings_health", callback.0)
    }
}
...
impl set_flags_for_admin_restore_all_buildings_health for super::SetReducerFlags {
    fn admin_restore_all_buildings_health(&self, flags: __ws::CallReducerFlags) {
        self.imp.set_call_reducer_flags("admin_restore_all_buildings_health", flags);
    }
}
