...
pub trait migrate_grant_default_collectibles {
    ...
    fn on_migrate_grant_default_collectibles(&self, callback: impl FnMut(&super::ReducerEventContext, ) + Send + 'static) -> MigrateGrantDefaultCollectiblesCallbackId;
    /// Cancel a callback previously registered by [`Self::on_migrate_grant_default_collectibles`],
    /// causing it not to run in the future.
    fn remove_on_migrate_grant_default_collectibles(&self, callback: MigrateGrantDefaultCollectiblesCallbackId);
}

impl migrate_grant_default_collectibles for super::RemoteReducers {
    fn migrate_grant_default_collectibles(&self, ) -> __sdk::Result<()> {
        self.imp.call_reducer("migrate_grant_default_collectibles", MigrateGrantDefaultCollectiblesArgs {  })
    }
    ...
    fn remove_on_migrate_grant_default_collectibles(&self, callback: MigrateGrantDefaultCollectiblesCallbackId) {
        self.imp.remove_on_reducer("migrate_grant_default_collectibles", callback.0)
    }
}
...
impl set_flags_for_migrate_grant_default_collectibles for super::SetReducerFlags {
    fn migrate_grant_default_collectibles(&self, flags: __ws::CallReducerFlags) {
        self.imp.set_call_reducer_flags("migrate_grant_default_collectibles", flags);
    }
}
