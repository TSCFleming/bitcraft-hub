...
fn biome_color(biome_id: u32) -> RGBColor {
    ...
    match biome_id_u8 {
        ...
        2 => RGBColor(12, 48, 11),         // Pine Woods - Dark Green
        3 => RGBColor(1, 1, 10),     // Snowy Peaks - Alice Blue
        4 => RGBColor(130, 230, 130),     // Breezy Plains - Light Green
        5 => RGBColor(210, 105, 30),      // Autumn Forest - Orange
        6 => RGBColor(100, 40, 60),       // Tundra - Pinky Purple
        ...
        13 => RGBColor(60, 100, 30),      // Jungle - Forest Green
        14 => RGBColor(100, 90, 108),      // Sapwoods - Sienna
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let total_count: i64 = client
        .query_one("SELECT count(*) FROM terrain_chunk_state", &[])?
        .get(0);
    let dimension_count: i64 = client
        .query_one(
            "SELECT count(*) FROM terrain_chunk_state WHERE \"dimension\" = $1",
            &[&dimension],
        )?
        .get(0);

    println!(
        "DB: {} | total rows: {} | dimension {} rows: {}",
        database_url, total_count, dimension, dimension_count
    );
...
WHERE "dimension" = $1"#,
    ...
    for row in rows {
        ...
        let biomes = json_array_to_u32_vec(&biomes);
        let water_levels = json_array_to_u32_vec(&water_levels);
        let water_types = json_array_to_u32_vec(&water_types);
        let original_elevations = json_array_to_u32_vec(&original_elevations);

        data.push((x, y, biomes, water_levels, water_types, original_elevations));
    }

    if data.is_empty() {
        return Err(
            format!(
                "No terrain chunks returned from Postgres (dimension={})",
                dimension
            )
            .into(),
        );
    ...
    let chunk_size = 1;
    let image_width = ((max_x - min_x + 1) as u32) * chunk_size * 32;
    let image_height = ((max_y - min_y + 1) as u32) * chunk_size * 32;
    ...
    for (x, y, biomes, water_levels, water_types, original_elevations) in data {
        for i in 0usize..1024 {
            ...
            let x_pos = ((x - min_x) as u32) * chunk_size * 32 + chunk_x;
            let y_pos = ((max_y - y) as u32) * chunk_size * 32 + (31 - chunk_y);
